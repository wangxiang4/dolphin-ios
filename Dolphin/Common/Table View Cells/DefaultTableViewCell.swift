...
class DefaultTableViewCell: TableViewCell {
    ...
    override func makeUI() {
        ...
        stackView.snp.remakeConstraints({ (make) in
            ...
            make.height.greaterThanOrEqualTo(Configs.BaseComponentDimensions.tableRowHeight)
        })
    }
    ...
    override func bind(to viewModel: TableViewCellViewModel) {
        ...
        // 设置图片hero动画匹配ID
        viewModel.imageUrl.asDriver().filterNil().drive(onNext: { [weak self] (url) in self?.leftImageView.hero.id = url }).disposed(by: rx.disposeBag)
    }
}
